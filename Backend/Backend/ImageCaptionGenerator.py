...
from fastai1.fastai.layers import Flatten, nn, AdaptiveConcatPool2d, requires_grad, F, random
...
class ImageCaptionGenerator(nn.Module):
    def __init__(self, device, filter_width, num_filters, vocab_size, emb_sz, out_seqlen, n_layers=3,
        ...
        self.rnn_dec = nn.GRU(num_filters + emb_sz, emb_sz, num_layers=n_layers,
                              dropout=0 if n_layers == 1 else p_drop)  # square to enable weight tying
...
class TeacherForcingCallback(Callback):
    ...
    def on_batch_begin(self, iteration: int, **kwargs: Any) -> None:
        self.learn.model.prob_teach_forcing = (
                                                          self.decay_iterations - iteration) * 1 / self.decay_iterations if iteration < self.decay_iterations else 0
